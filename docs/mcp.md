...
**기기마다 하나씩 발급한다.** 계정당 여러 개를 둘 수 있어서, 새 PC를 붙일 때 이미 연결된
기기를 끊지 않아도 되고 잃어버린 기기 것만 골라 지울 수 있다.

1. 로그인한 상태로 `/settings` 접속
2. 기기 이름(예: `집 PC`, `노트북`)을 적고 **"토큰 발급"**
3. 값은 화면에 **한 번만** 표시된다. 안전한 곳에 옮겨 적는다 - 해시만 저장하므로 나중에
   다시 볼 수 없다
4. 목록에서 마지막 사용 시각을 보고, 안 쓰는 것은 **삭제**한다. 지우면 그 기기의 연결만
   즉시 끊긴다
