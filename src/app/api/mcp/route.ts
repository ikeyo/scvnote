    ...
    case "initialize": {
      ...
      return rpcResult(id, {
        ...
        instructions:
          ...
          "작업일지(kind=WORKLOG)를 쓰거나 이어쓸 때는 그 글 첫 줄에 " +
          "`## 빌드 #<커밋수> · <짧은해시> · <시각>` 을 넣는다 - 나중에 빌드 번호로 되짚기 위해서다. " +
          "커밋수는 `git rev-list --count HEAD`, 짧은해시는 `git rev-parse --short HEAD`로 구한다. " +
          "빠뜨리면 저장이 거부되고 무엇을 넣어야 하는지 응답으로 알려준다.",
      });
    }
