...
| 인증 | 1인용 세션 쿠키 (scrypt + JWT). 회원가입 없음 |
| Claude 연동 | MCP Streamable HTTP 엔드포인트 |
...
| 화면 | 하는 일 |
|---|---|
| `/login` | 최초 1회 계정 생성, 이후 로그인 |
| `/projects` | 프로젝트 생성·수정·보관·삭제 |
| `/todos` | 프로젝트별 할 일. 오류·개선·아이디어를 진행 체크 |
| `/tags` | 태그 이름 변경·합치기·삭제·미사용 정리 |
| `/notes` | 프로젝트/카테고리별 목록 · 검색 |
| `/notes/[id]` | TipTap 에디터. 자동 저장, 태그, 스크린샷 `Ctrl+V` 첨부 |
| `/secrets` | 비밀번호 보관함. 브라우저에서만 복호화 |
| `/api/mcp` | Claude Code / 데스크톱 앱 연동 |
